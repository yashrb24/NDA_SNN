...
parser.add_argument(
    "--seed", default=1000, type=int, help="seed for initializing training. "
)
parser.add_argument(
    "-T",
    "--time",
...
parser.add_argument(
    "--nda", action="store_true", help="if use neuromorphic data augmentation."
)
parser.add_argument(
    "--fold_idx",
    default=0,
    type=int,
    metavar="N",
    help="fold index for dvscifar dataset",
)
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.Adam(
        model.parameters(), lr=args.lr / 256 * args.batch_size, weight_decay=1e-4
    )
    ...
    for epoch in range(args.epochs):
        ...
        print(
            "Epoch:[{}/{}]\t loss={:.5f}\t acc={:.3f},\t time elapsed: {}".format(
                epoch, args.epochs, loss, acc, t_diff
            )
        )
        scheduler.step()
